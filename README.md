...
├── OWNS_run_first_transition_latest.m
...
│   │
│   ├── ensemble/
│   │   ├── assembleBlockDiagonalFamily.m
│   │   ├── buildEnsembleTransitionThreshold.m
│   │   ├── discoverOWNSEnsembleFiles.m
│   │   ├── downsampleOWNSStations.m
│   │   ├── interpMatrixFamilyToGrid.m
│   │   ├── loadOWNSModeCached.m
│   │   ├── loadOWNSModeRaw.m
│   │   ├── reduceEnsembleMode.m
│   │   └── runOWNSEnsembleWorkflow.m
│   │
│   ├── matrices/
│   │   ├── buildAFromOWNS.m
│   │   ├── buildGFamily.m
...
cfg.owns.ensembleReferenceFile = [];   % mode whose grid defines the common x
...
or the full driver script:

```matlab
OWNS_run_first_transition_ensemble
```
...
```matlab
runOWNSEnsembleSmokeTest
```

Runs the multi-mode total-energy workflow against every OWNS file in `data/`, and verifies that a cold-cache run and a warm-cache run reuse the cache and reproduce identical probability curves.
...
- broadband joint reconstruction across multiple \((\omega,\beta)\) pairs (the ensemble workflow combines total energy across modes, but does not reconstruct a joint physical field);
...
7. design joint reconstruction over the full spectral space.
